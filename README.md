...
### Introduction ###
This console-based C# application accepts input text either from a file or directly via the command line, and encrypts it using the Data Encryption Standard (DES) algorithm. The application displays the resulting encrypted data, then performs decryption to show the original text.

DES encryption follows PKCS#7 padding to 16 bytes, even though DES operates on 8-byte blocks. No salt or initialization vector is used in the encryption process
...
The modular architecture is implemented from the following hierarchy in Program.cs:  
...
│   │        ├── EncryptBlock(byte[] block, byte[] key): byte[]  
...
│        └── ReadInputStream(string path): Stream  
│        └── WriteOutputStream(string path): void     
...
- Dependency Inversion (DI): high-level modules depend on abstractions not on low-level concrete implementations

### Testability ###  
- Small interfaces + DI make unit tests easier to construct. Fake `IFileProcessor` and `IDesCipher` are used in tests to validate CLI behavior without touching disk or running full crypto.
...
- Tests run via the Test Explorer or dotnet test in CI.
...
- Streaming API for I/O avoids loading large files into memory. The `ReadInputStream` / `WriteOutputStream` methods allow processing arbitrarily large files block by block
...
- Replace DES for security: because the system is wired to `IDesCipher`, switching to `AesCipher` or `TripleDesCipher` is a straightforward change - no CLI or I/O code must be rewritten

### Maintainability and next steps ###  
- Add CI to run tests on automatically on each push/pull request, static analyzers, and code coverage
- Document key format, CLI conventions (- for stdin/stdout, --key-format), and security caveats (DES is deprecated)

### DES is for demonstration purposes only ###  
- For production usage, platform crypto (AES-GCM) and authenticated modes should be used   

### One-line summary ###  
- `DesCli` is intentionally modular: small, focused classes and interfaces give the freedom to swap cryptography, scale I/O, and write reliable unit tests - all aligned with SOLID design and modern .NET practices